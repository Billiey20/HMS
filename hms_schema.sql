...
CREATE TABLE IF NOT EXISTS patients (
  id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  patient_no        VARCHAR(20) UNIQUE NOT NULL,  -- e.g. BP-00001
  first_name        VARCHAR(100) NOT NULL,
  middle_name       VARCHAR(100),
  last_name         VARCHAR(100) NOT NULL,
  date_of_birth     DATE,
  age               VARCHAR(20),  -- 'Adult', 'Child', or numeric
...
CREATE TABLE IF NOT EXISTS opd_visits (
  ...
  triage_priority       VARCHAR(20) DEFAULT 'normal',  -- 'emergency','urgent','normal'
  -- Vitals
  ...
  department_id         UUID REFERENCES departments(id),
  -- Status
  status                VARCHAR(30) DEFAULT 'waiting',  -- 'waiting','in_consultation','completed','admitted'
  queue_no              INTEGER,
...
CREATE TABLE IF NOT EXISTS consultations (
  ...
  -- Decision
  decision              VARCHAR(30) DEFAULT 'discharge',  -- 'discharge','admit','refer','review'
...
CREATE TABLE IF NOT EXISTS wards (
  ...
  name            VARCHAR(100) UNIQUE NOT NULL,
  department_id   UUID REFERENCES departments(id),
  ward_type       VARCHAR(50),  -- 'general','maternity','surgical','paediatric','icu'
...
-- Daily nursing/doctor notes
CREATE TABLE IF NOT EXISTS clinical_notes (
  id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  admission_id  UUID NOT NULL REFERENCES admissions(id) ON DELETE CASCADE,
  patient_id    UUID NOT NULL REFERENCES patients(id),
  written_by    UUID NOT NULL REFERENCES users(id),
  note_type     VARCHAR(50) DEFAULT 'nursing',  -- 'nursing','doctor','social_work'
  note          TEXT NOT NULL,
  created_at    TIMESTAMPTZ DEFAULT NOW()
);

-- Vitals chart (inpatient)
...
CREATE TABLE IF NOT EXISTS drug_catalog (
  ...
  formulation     VARCHAR(100),  -- 'tablet','syrup','injection','cream'
  unit            VARCHAR(50),   -- 'tab','vial','ml','g'
...
-- General hospital inventory (non-pharmaceutical)
...
CREATE TABLE IF NOT EXISTS bill_items (
  ...
  category      VARCHAR(100),  -- 'consultation','lab','pharmacy','ward','procedure'
...
CREATE TABLE IF NOT EXISTS payments (
  ...
  method          VARCHAR(50),  -- 'cash','mpesa','card','insurance','waiver'
...
-- Allow all authenticated users read access to patients (doctors, nurses, billing all need it)
...
-- Current inpatients with bed & ward info
